...
def gendaylit(altitude, month, day, hour, directirradiance, diffuseirradiance,
    ...
    skyclearness = sky_clearness(
        diffuseirradiance, directirradiance, sunzenith)

    skyclearness, skybrightness = check_parametrization(
    ...
    skybrightness = \
        coeff_lum_perez(radians(sunzenith), skyclearness,
                        skybrightness, coeff_perez)
    ...
    for j in xrange(145):
        ...
        v = calc_rel_lum_perez(
            dzeta, gamma, radians(
                sunzenith), skyclearness, skybrightness, coeff_perez
        )
...
def theta_phi_to_dzeta_gamma(theta, phi, z):
    ...
    if ((math.cos(z) * math.cos(theta) + math.sin(z) * math.sin(theta) *
        ...
        gamma = 0
            ...
            math.cos(phi) > 1.1:
        raise ValueError(
            "error in calculation of gamma (angle between point and sun)")
    else:
...
def calc_rel_lum_perez(dzeta, gamma, z, epsilon, delta, coeff_perez):
    ...
    if epsilon < skyclearinf or epsilon >= skyclearsup:
        raise ValueError(
            "Error: epsilon out of range in function calc_rel_lum_perez!\n")
...
def glob_h_diffuse_effi_perez(skyclearness, skybrightness, sunzenith):
    ...
    category_bounds = (1, 1.065, 1.230, 1.500, 1.950,
                       2.800, 4.500, 6.200, 12.01)
...
def check_input_values(directilluminance, diffuseilluminance, altitude):
    ...
    if directilluminance > solar_constant_l:
        raise ValueError(
            "Warning: direct illuminance exceeds solar constant\n")
...
def coeff_lum_perez(z, epsilon, delta, coeff_perez):
    ...
    if epsilon < skyclearinf or epsilon >= skyclearsup:
        raise ValueError(
            "Error: epsilon out of range in function coeff_lum_perez!\n")
...
month = 10
day = 10
directirradiance = 0.5
diffuseirradiance = 0.1
print(gendaylit(altitude, month, day, day, directirradiance, diffuseirradiance,
                output_type=0))
